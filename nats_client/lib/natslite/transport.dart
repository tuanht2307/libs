  ...
  Future<void> connect(List<String> servers) async {
    ...
    _subscription = _socket!.stream.listen((data) async {
      ...
      if (_openSubscription != null) {
        ...
        debug('NATS::callback buffer.length(${o.buffer.length})');
        if (o.buffer.length >= o.totalBytes + 2) {
          // end message [_CR, _LF]
          final result = SubscriptionResult(o.buffer.sublist(0, o.totalBytes), o.subscription, o.subject);
          debug('NATS::callback(${result.subject})');
          o.subscription.callback!(result);
    ...
    switch (command ?? '') {
      ...
      case _Command.HMSG:
        ...
        if (subs[sid] != null) {
          ...
          debug('NATS::callback message size: ${start + totalBytes}, payload (${totalBytes}), buffer: ${data.length}, remain data: ${data.length - start - totalBytes-2}');
          if (totalBytes > 0 && s.callback is SubCallback) {
            if (start + totalBytes < data.length) {
//start + totalBytes -> data.length;
              final result = SubscriptionResult(data.sublist(start, start + totalBytes), s, subject);
              debug('NATS::callback(${result.subject}), ${data.length}');
              s.callback!(result);
              
              if((start + totalBytes +2) < data.length){
                final remainData = data.sublist(start + totalBytes+2);
                debug('NATS::callback(${utf8.decode(remainData)}');
                debug('NATS::remain data(${remainData.hashCode})');
                parse(remainData);
              }
            } else {
